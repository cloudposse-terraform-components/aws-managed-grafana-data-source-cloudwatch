...
| <a name="input_assume_role_arn"></a> [assume\_role\_arn](#input\_assume\_role\_arn) | IAM Role ARN to assume for cross-account CloudWatch access. If empty, uses the Grafana workspace's IAM role. | `string` | `""` | no |
| <a name="input_attributes"></a> [attributes](#input\_attributes) | ID element. Additional attributes (e.g. `workers` or `cluster`) to add to `id`,<br/>in the order they appear in the list. New attributes are appended to the<br/>end of the list. The elements of the list are joined by the `delimiter`<br/>and treated as a single ID element. | `list(string)` | `[]` | no |
| <a name="input_cloudwatch_account_id"></a> [cloudwatch\_account\_id](#input\_cloudwatch\_account\_id) | AWS Account ID where CloudWatch logs are stored (the account to query) | `string` | `""` | no |
| <a name="input_cloudwatch_region"></a> [cloudwatch\_region](#input\_cloudwatch\_region) | AWS Region where CloudWatch logs are stored. Defaults to the component's region if not specified. | `string` | `""` | no |
...
| <a name="input_datasource_name"></a> [datasource\_name](#input\_datasource\_name) | Name for the CloudWatch data source in Grafana. Defaults to the component ID if not specified. | `string` | `""` | no |
| <a name="input_default_log_groups"></a> [default\_log\_groups](#input\_default\_log\_groups) | List of default log groups to make available in Grafana | `list(string)` | `[]` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_id"></a> [id](#output\_id) | The full ID of this CloudWatch data source (orgId:uid) |
| <a name="output_name"></a> [name](#output\_name) | The name of this CloudWatch data source |
| <a name="output_uid"></a> [uid](#output\_uid) | The UID of this CloudWatch data source |
...
- [Amazon CloudWatch](https://docs.aws.amazon.com/cloudwatch/) - AWS CloudWatch monitoring service

- [Amazon Managed Grafana](https://docs.aws.amazon.com/grafana/) - Managed Grafana service

- [Grafana CloudWatch Data Source](https://grafana.com/docs/grafana/latest/datasources/cloudwatch/) - Grafana CloudWatch plugin documentation
...
[<img src="https://cloudposse.com/logo-300x69.svg" height="32" align="right"/>](https://cpco.io/homepage?utm_source=github&utm_medium=readme&utm_campaign=cloudposse-terraform-components/aws-managed-grafana-data-source-cloudwatch&utm_content=)
